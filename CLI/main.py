...
class PlantAnalyzer:
    def __init__(self, model_path, pixels_per_mm=11.5):
        """
        model_path: путь к лучшей модели (best.pt)
        pixels_per_mm: коэффициент калибровки (по умолчанию 11.5)
        """
        ...
        self.original_pixels_per_mm = pixels_per_mm
        self.pixels_per_mm = pixels_per_mm

        # Истинные имена классов в модели
    ...
    def create_overlay_image(self, image, masks_data):
        ...
        try:
            ...
            print(f"Ошибка создания overlay: {e}")
            return ""

    # ---------- основной метод анализа ----------
    def analyze_image(self, image_path, save_dir='results'):
        ...
        if max(original_width, original_height) > max_size:
            ...
            # Коррекция калибровки с учетом масштаба
        ...
        if result.masks is not None:
            ...
            # Сохраняем визуализацию YOLO
            vis_path = os.path.join(save_dir, f'{timestamp}_result.jpg')
            ...
            # Формируем результат для фронтенда
            result_data = {
                'leaf': {
                    'polygon': [],
                    'mask': '',
                    'length': metrics['root']['length_mm'],  # Длина листьев
                    'area': metrics['root']['area_mm2'],  # Площадь листьев
                    'confidence': 0,
                ...
                'root': {
                    'polygon': [],
                    'mask': '',
                    'length': metrics['stem']['length_mm'],  # Длина корней
                    'area': metrics['stem']['area_mm2'],  # Площадь корней
                    'confidence': 0,
                ...
                'stem': {
                    'polygon': [],
                    'mask': '',
                    'length': metrics['leaf']['length_mm'],  # Длина стеблей
                    'area': metrics['leaf']['area_mm2'],  # Площадь стеблей
                    'confidence': 0,
                    'count': model_class_counts[2]
                }
            }

            # Заполняем маски и confidence
            for obj in masks_data:
            ...
            # Вывод результатов с длиной
            print(f"\n📊 РЕЗУЛЬТАТЫ ИЗМЕРЕНИЙ:")
            print(f"   ЛИСТЬЯ (из class 0):")
            print(f"     Площадь: {result_data['leaf']['area']:.2f} мм²")
            print(f"     Длина: {result_data['leaf']['length']:.2f} мм")
            print(f"     Количество: {result_data['leaf']['count']}")
            print(f"   КОРНИ (из class 1):")
            print(f"     Площадь: {result_data['root']['area']:.2f} мм²")
            print(f"     Длина: {result_data['root']['length']:.2f} мм")
            print(f"     Количество: {result_data['root']['count']}")
            print(f"   СТЕБЛИ (из class 2):")
            print(f"     Площадь: {result_data['stem']['area']:.2f} мм²")
            print(f"     Длина: {result_data['stem']['length']:.2f} мм")
            print(f"     Количество: {result_data['stem']['count']}")
...
def find_model_file():
    """Поиск файла модели в текущей директории"""
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Анализ изображений растений')
    parser.add_argument('-i', '--input', required=True, help='Папка с входными изображениями')
    parser.add_argument('-o', '--output', default='results', help='Папка для результатов')
    parser.add_argument('-m', '--model', help='Путь к файлу модели (best.pt)')
    parser.add_argument('-p', '--pixels-per-mm', type=float, default=11.5,
                        help='Калибровка пикс/мм (по умолчанию: 11.5)')

    args = parser.parse_args()

    print("=" * 60)
    print("🌱 АНАЛИЗ РАСТЕНИЙ — ПШЕНИЦА И РУККОЛА")
    print("=" * 60)
    ...
    if not model_path:
        ...
        if not model_path:
            ...
            print("   Укажите путь к модели через параметр -m")
            print("   или поместите файл best.pt в текущую папку")
            exit(1)
    ...
    print(f"📏 Калибровка: {PIXELS_PER_MM:.2f} пикс/мм")

    # Инициализация анализатора
    analyzer = PlantAnalyzer(model_path, pixels_per_mm=PIXELS_PER_MM)
    ...
    test_files = glob.glob(os.path.join(args.input, '*.jpg')) + \
                 glob.glob(os.path.join(args.input, '*.png')) + \
                 glob.glob(os.path.join(args.input, '*.jpeg'))

    print(f"\n📁 Найдено изображений: {len(test_files)}")
    print("=" * 60)

    all_results = []
    for img_path in test_files:
        result = analyzer.analyze_image(img_path, save_dir=args.output)
        if result:
    ...
    if all_results:
        ...
        total_leaf_area = sum(r['leaf']['area'] for r in all_results)
        total_root_area = sum(r['root']['area'] for r in all_results)
        total_stem_area = sum(r['stem']['area'] for r in all_results)

        total_leaf_length = sum(r['leaf']['length'] for r in all_results)
        total_root_length = sum(r['root']['length'] for r in all_results)
        total_stem_length = sum(r['stem']['length'] for r in all_results)

        total_leaf_count = sum(r['leaf']['count'] for r in all_results)
        total_root_count = sum(r['root']['count'] for r in all_results)
        total_stem_count = sum(r['stem']['count'] for r in all_results)

        print(f"Всего изображений: {len(all_results)}")
        print(f"\n📊 ЛИСТЬЯ:")
        print(f"   Суммарная площадь: {total_leaf_area:.2f} мм²")
        print(f"   Суммарная длина: {total_leaf_length:.2f} мм")
        print(f"   Всего объектов: {total_leaf_count}")
        print(f"\n📊 КОРНИ:")
        print(f"   Суммарная площадь: {total_root_area:.2f} мм²")
        print(f"   Суммарная длина: {total_root_length:.2f} мм")
        print(f"   Всего объектов: {total_root_count}")
        print(f"\n📊 СТЕБЛИ:")
        print(f"   Суммарная площадь: {total_stem_area:.2f} мм²")
        print(f"   Суммарная длина: {total_stem_length:.2f} мм")
        print(f"   Всего объектов: {total_stem_count}")
